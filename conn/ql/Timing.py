...
class Timing:
    ...
    def check_ct(self, state=0) -> str:
        ...
        """
        li = ""
        value1 = self.sql.selectAll(table=self.sql.surface[3], where=f"state={state}")
        for ql_tk in value1:
            for i in range(3):
                ck = self.ql.ql_tk(ql_tk)
                if ck[0] == 200:
                    self.sql.update(table=self.sql.surface[3], Authorization=ck[1], state=0,
                                    where=f"name='{ql_tk[0]}'")
                    self.logger.write_log(f"{ql_tk[0]} 获取tk成功")
                    break
                # 两次获取不到删除
                elif i == 2 or ck[0] == 403:
                    self.sql.delete(table=self.sql.surface[3], where=f"name='{ql_tk[0]}'")
        ...
        if li:
            # 这一点是为了检测标记是否需要更新
            self.interaction.marking_time()
            self.interaction.for_message(li, False)
        return li
    ...
    def clear_list(self, state=0):
        ...
        :return: 返回删除的字符串
        """
        self.sql.delete(table=self.sql.surface[1])
        try:
            li = ""
            value1 = self.sql.selectAll(table=self.sql.surface[3], where=f"state={state}")
            for ql_tk in value1:
                js_ql = self.ql.crons(ql_tk)
                # 跳过检测
                if js_ql[0] != 200:
                    # 获取到非正常状态自动删除
                ...
                # 执行到这里把异常的改为正常
                if state == 1:
                    self.sql.update(table=self.sql.surface[3], state=0, where=f"name='{ql_tk[0]}'")
                js = dict()
                # 如果青龙里面有层data就解包
